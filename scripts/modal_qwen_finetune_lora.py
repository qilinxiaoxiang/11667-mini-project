"""
LoRA Fine-tuning script for Qwen2.5-0.5B on Modal
- Model: Qwen/Qwen2.5-0.5B
- Dataset: Hierarchical medical responses (3,311 samples, up to 768 tokens)
- GPU: A100-80GB required (seq_len² scaling: 768 tokens requires more memory)
- Batch Size: 24 (effective 48 with gradient accumulation)
...
app = modal.App("medical-response-lora-qwen")
...
def finetune_qwen_lora(
    ...
    """
    LoRA Fine-tune Qwen2.5-0.5B on hierarchical medical dataset

    Memory Estimation (with batch_size=24, seq_length=768):
    - Base model weights (frozen, FP16): 0.93 GB
    - LoRA parameters (trainable): ~0.05 GB (r=8, ranks on all layers)
    - Optimizer states (AdamW): ~0.2 GB
    - Activations (attention: 24 × 14 heads × 768²): ~5-6 GB (quadratic with seq_len)
    - Total: ~55-60 GB (higher utilization, still safe on A100-80GB)

    LoRA Configuration:
    - r=8: Rank of LoRA matrices
    - lora_alpha=32: Scaling factor
    - target_modules: q_proj, v_proj (critical for attention)
    """
    ...
    lora_config = LoraConfig(
        ...
        target_modules=["q_proj", "v_proj"],  # Qwen specific attention layers
    ...
    output_dir = f"/models/qwen_lora_{timestamp}"
    ...
    training_args = TrainingArguments(
        ...
        fp16=True,  # Use float16 for training (more widely supported than bf16)
        save_total_limit=3,
        push_to_hub=False,
