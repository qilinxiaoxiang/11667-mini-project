...
We formulate the hierarchical medical response generation as a text-generation task. Our training data is derived from the \href{https://huggingface.co/datasets/mahfoos/Patient-Doctor-Conversation}{Patient-Doctor-Conversation} dataset on HuggingFace (mahfoos/Patient-Doctor-Conversation), which we processed into hierarchical structure using the DeepSeek API.
...
\begin{itemize}
    \item Full fine-tuning: 5 epochs with early stopping (patience=2)
    \item LoRA: 5-10 epochs depending on model
\end{itemize}
...
To assess overfitting, we need to examine the gap between training and validation loss during fine-tuning. The evaluation results on 40 unseen test samples provide indirect evidence of generalization:

\begin{itemize}
    \item \textbf{Full fine-tuned models} achieve high MECE scores (92-95\% compliance) on test data, suggesting good generalization
    \item \textbf{LoRA models} show significantly degraded test performance (47.5-62.5\% MECE compliance), but it indicates insufficient learning capacity since validation loss was not abnormally higher than training loss.
\end{itemize}
